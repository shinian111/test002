    ...
    class FaultDataLoader {
        ...
        async loadMainData() {
            try {
                ...
                this.treeLoading.innerHTML = '<p>加载故障数据失败，请检查网络连接或控制台错误信息</p>';
            }
        }
        ...
        async loadJSONFile(filePath) {
            ...
            try {
                ...
                if (filePath.startsWith('/')) {
                    fullPath = `${this.basePath}${filePath.substring(1)}`;
                } else if (filePath.startsWith('./')) {
                    fullPath = `${this.basePath}${filePath.substring(2)}`;
                } else {
                    fullPath = `${this.basePath}${filePath}`;
                }
                
                console.log('正在加载文件:', fullPath);
        ...
        async displayNodeContent(node) {
            ...
            if (!nodeData) {
                ...
                this.showError('未找到节点数据');
                return;
            }
            
            console.log('显示节点内容:', nodeData);
            
            // 显示或隐藏注意事项
            if (nodeData.note) {
                this.noteContent.innerHTML = nodeData.note;
                this.noteBox.style.display = 'block';
                this.currentNote = nodeData.note;
            } else {
            ...
            try {
                ...
                // 如果有外部文件，加载文件内容
                if (nodeData.file) {
                    console.log('加载外部文件:', nodeData.file);
                    displayData = await this.loadJSONFile(nodeData.file);
                    console.log('文件加载成功:', displayData);
                }
                ...
                this.showError(`加载故障详情失败: ${error.message}`);
            }
        }
            ...
            while (parent && !foundNote) {
                ...
                if (parentNode) {
                    ...
                    if (parentData && parentData.note) {
                        ...
                        this.noteBox.style.display = 'block';
                        this.currentNote = parentData.note;
                        foundNote = true;
                    }
                }
        ...
        updateInfoDisplay(data) {
            if (!data) {
                this.infoDisplay.innerHTML = '<p>无可用数据</p>';
                return;
            }
            
            const formattedMeasures = data.measures ? 
                data.measures.replace(/\n/g, '<br>') : 
                '<p style="color: #999; font-style: italic;">暂无维修措施信息</p>';
            
            const formattedRootCause = data.rootCause ? 
                data.rootCause.replace(/\n/g, '<br>') : 
                '<p style="color: #999; font-style: italic;">暂无根本原因信息</p>';
